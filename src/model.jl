...
function MOI.add_constrained_variables(
    ...
    cidx = [
        _create_constraint(model, vidx[i], sets[i], true) for
        i in 1:length(sets)
    ]
    return vidx, cidx
end

function MOI.add_constrained_variable(model::Model, set::MOI.AbstractScalarSet)
...
function MOI.get(model::Model, ::MOI.ListOfConstraintIndices{F,S}) where {F,S}
    return [
        c.index for
